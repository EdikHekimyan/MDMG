...
endBtn.addEventListener('click', () => {
  ...
  if (activeWin) {
    ...
    winDiv.innerHTML = `<p>Your amount</p><p>"${visibleAmount}"</p>`
    startBtn.insertAdjacentElement('afterEnd', winDiv)
    setTimeout(() => {
      winDiv.classList.replace('animate__fadeIn', "aniamte__fadeOut")
...
helpFriend.addEventListener('click', function getHelpFriend() {
...
helpAI.addEventListener('click', async function getHelpAI() {
  ...
  try {
    ...
    if (aiIndex === -1) {
      aiIndex = answerOptions.fillIndex(opt => opt.startsWith(aiResult.answer.charAt(0)))
    }
    for (let i = 0; i < blockActiveQuestionChild.children.length; i++) {
    ...
    aiExplainText.innerText = 'Failed to obtain answer from AI'
    aiExplainBlock.classList.add('show')
  }
})
  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'You help with the game "Million Dollar Worth Mind Game".' +
            'Select the correct answer from the provided options and explain in short (2-3 sentences) why it is correct.' +
            'Answer EXCLUSIVELY with JSON format: {"answer": "<option full text>", "explanation": "<explanation>"}, nothing else.'
        },
        {
          role: 'user',
...
function getBlockBefore(block) {
  block.insertAdjacentHTML('beforeBegin', `<div class="user-win animate__animated animate__fadeIn"><p>Your amount</p><p>"${getGuarantWin()}"</p></div>`);
}
...
const answers = {
  ...
  question3: "D:",
  question4: "B: macOS",
  question5: "A: Panthera",
  question6: "D: PHP",
  question7: "C: Programming",
  question8: "B: All-in-one PC",
  question9: "C: Ether",
  question10: "A: Lion",
  question11: "D: All of the above",
  question12: "C: Genetic mutation of cheetah",
  question13: "B: Apple",
  question14: "C: Beta",
  question15: "A: Peripheral for typing",
  questionextra: "A: Malicious clickbait"
}
